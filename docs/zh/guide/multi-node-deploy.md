...
::: tip 集群内所有节点架构必须保持一致
单个 Cube Sandbox 集群目前不支持 x86_64 与 aarch64 节点混部 —— 发布包中的 guest 内核和 `vmlinux` 与架构强相关。如需同时支持两种架构，请分别构建并发布对应架构的发布包。在 aarch64 控制 / 计算节点上，还需在 `.env` 中覆盖依赖服务的容器镜像为 arm64 版本（详见[裸金属 / 物理机部署 — aarch64 附加配置](./bare-metal-deploy.md#aarch64-附加配置)）。
:::
