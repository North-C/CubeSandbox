...
#[instrument]
fn mount_to_rootfs_with_timeout(logger: &Logger, m: &InitMount, timeout: Duration) -> Result<()> {
    let options_vec: Vec<&str> = m.options.clone();
    let (flags, options) = parse_mount_flags_and_options(options_vec);
    ...
    baremount_with_timeout(source, dest, m.fstype, flags, &options, logger, timeout).or_else(|e| {
        if m.src != "dev" {
            return Err(e);
        }
        Ok(())
