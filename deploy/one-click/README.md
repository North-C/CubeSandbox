...
The fixed guest kernel is selected by target architecture. Set the target
architecture explicitly when building for a machine that differs from the build
host:

```bash
export ONE_CLICK_TARGET_ARCH=amd64  # or arm64
```

By default, the kernel is read from one of these paths:

- `assets/kernel-artifacts/linux-amd64/vmlinux`
- `assets/kernel-artifacts/linux-arm64/vmlinux`

For backward-compatible amd64 builds, `assets/kernel-artifacts/vmlinux` is still
accepted. The kernel path can also be overridden via an environment variable:
...
- Locally built `cube-image-linux-<arch>/cube-guest-image-cpu.img`
...
- `cube-kernel-scf-linux-<arch>.zip` packaged on the fly from `vmlinux`
...
- If the target kernel is missing from `assets/kernel-artifacts/linux-<arch>/vmlinux`, `build-vm-assets.sh` and `build-release-bundle.sh` will fail immediately. The `cube-kernel-scf-linux-<arch>.zip` in the release package is generated automatically during the packaging phase.
