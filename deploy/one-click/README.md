...
- On SELinux-enabled targets with `restorecon`, the installer automatically relabels the installation directory, systemd units, and one-click commands under `/usr/local/bin` before starting the systemd target. This prevents copied scripts from retaining temporary-file contexts such as `user_tmp_t`, which can otherwise make systemd fail with `203/EXEC`.
...
- The `cube proxy` build context renders a local compose file. The proxy base image is architecture-aware: `amd64` defaults to the Tencent OpenResty mirror and `arm64` defaults to `openresty/openresty:1.21.4.1-6-alpine-fat`.
- The CoreDNS image is architecture-aware: `amd64` defaults to the Tencent CoreDNS mirror and `arm64` defaults to `coredns/coredns:1.14.2`. If `resolvectl` is available, one-click creates a dedicated dummy link (default `cube-dns0`) with a local address, binds CoreDNS to `169.254.254.53` on that link by default, and routes `cube.app` through the link without affecting the host's default public DNS path. If `resolvectl` is unavailable, the installer first reuses an existing active host `dnsmasq.service` that listens on `127.0.0.1:53`, asks it to additionally listen on `169.254.254.53`, and forwards `cube.app` to local CoreDNS. If no such host dnsmasq stub service is available, it falls back to `NetworkManager + dnsmasq`. Both fallback paths rewrite `/etc/resolv.conf` to point at the same non-loopback IP, avoiding Docker's fallback to public DNS when `/etc/resolv.conf` contains only loopback nameservers.
...
- DNS split routing: `resolvectl`, or `systemctl + active dnsmasq.service` listening on `127.0.0.1:53`, or `systemctl + NetworkManager`.
...
- The target machine preferentially uses `systemd-resolved` / `resolvectl` for split DNS of `cube.app`. The current implementation creates a dedicated dummy link (default `cube-dns0`), assigns it a local `/32` address, binds CoreDNS to `169.254.254.53` on that link by default, and attaches that address plus `~cube.app` to the link. If that capability is unavailable, the installation script first reuses an existing active host `dnsmasq.service` that listens on `127.0.0.1:53`: it writes `/etc/dnsmasq.d/90-cubeproxy-cube-app.conf`, asks dnsmasq to listen on both `127.0.0.1` and `169.254.254.53`, and forwards `cube.app` to `127.0.0.54`. If no such host dnsmasq stub service is available, it falls back to `NetworkManager + dnsmasq`. The fallback paths point `/etc/resolv.conf` at `169.254.254.53`, so host applications and Docker containers see the same non-loopback resolver.
...
- If the target machine has neither `systemd-resolved` / `resolvectl`, an active `dnsmasq.service`, nor a restartable `NetworkManager`, one-click will currently report an error.
...
- Verify the local CoreDNS path: on all paths, the client entry point is the same dummy-link IP, so run `dig +tcp +timeout=3 foo.cube.app @169.254.254.53`. CoreDNS itself stays bound to `127.0.0.54` internally; the `systemd-resolved` path talks to CoreDNS directly, while the `system-dnsmasq` and `NetworkManager + dnsmasq` fallback paths go through dnsmasq.
