...
guest kernel 会按目标架构选择。如果构建目标架构和构建机不同，请显式设置：

```bash
export ONE_CLICK_TARGET_ARCH=amd64  # 或 arm64
```
...
兼容 amd64 历史构建时，仍接受 `assets/kernel-artifacts/vmlinux`。也可以通过环境变量覆盖：
...
- 本地构建得到的 `cube-image-linux-<arch>/cube-guest-image-cpu.img`
- `cubeproxy/` 目录及其 `build-context`
- `support/` 目录及其 compose 模板
- `webui/` 目录、compose 模板、nginx 配置和已构建的 `web/dist` 静态资源
- 基于 `vmlinux` 现场打包得到的 `cube-kernel-scf-linux-<arch>.zip`
...
- 如果缺少 `assets/kernel-artifacts/linux-<arch>/vmlinux`，`build-vm-assets.sh` 和 `build-release-bundle.sh` 会立即失败；发布包里的 `cube-kernel-scf-linux-<arch>.zip` 会在打包阶段自动生成。
